...
public class AudioEngine {
    ...
    public func render(duration: Double) -> AVAudioPCMBuffer {
        ...
        let strSmplCount = Int(avEngine.manualRenderingSampleTime)
        ...
        do {
            while avEngine.manualRenderingSampleTime < sampleCount + strSmplCount {
                let currentSampleCount = Int(avEngine.manualRenderingSampleTime)
                let framesToRender = min(UInt32(sampleCount + strSmplCount - currentSampleCount), maximumFrameCount)
