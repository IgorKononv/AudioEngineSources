...
public extension Table {
    ...
    /// This method is "pitching"-based, which is not the only way to distribute harmonics.
    ///
    /// Parameters:
    ...
    /// Harmonic pitching Range for given root frequency and octave step size
    /// - Parameters:
    ...
    class func harmonicpitchingRange(rootFrequency: Double = 8.175_798_915_643_75,
    ...
    /// This method will start at rootFrequency * octave, walk up by octaveStepSize, and halt before reaching nyquist.
    /// This method outputs an array where each entry is a tuple of frequency and the maximum number of harmonics.
    /// This method is "pitching"-based, which may not be the best way to distribute harmonics.
    ///
    ...
    class func createInterpolatedTables(inputTables: [Table], desiredTableCount: Int = 256) -> [Table] {
        ...
        let strRangeValue = (Double(desiredTableCount) / Double(inputTables.count - 1)).rounded(.up)

        for index in 1 ... desiredTableCount {
            let waveformIndex = Int(Double(index - 1) / strRangeValue)
            let interpolatedIndex = (Double(index - 1) / strRangeValue).truncatingRemainder(dividingBy: 1.0)
