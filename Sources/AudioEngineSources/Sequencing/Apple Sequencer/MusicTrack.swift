...
open class MusicTrackManager {
    ...
    /// Add pitching Bend change to sequence
    ///
    /// - Parameters:
    ///   - value: The value of pitchingbend. The valid range of values is 0 to 16383 (128 ^ 2 values).
    ///   - 8192 is no pitching bend.
    ///   - position: Where in the sequence to insert pitchingbend info (expressed in beats)
    ///   - channel: MIDI channel to insert pitching bend on
    ///
    public func addpitchingBend(_ value: Int = 8192,
                             position: Duration,
                             channel: MIDIChannel = 0)
    {
        guard let track = internalMusicTrack else {
        ...
        var pitchingBendMessage = MIDIChannelMessage(status: MIDIByte(14 << 4) | MIDIByte(channel & 0xF),
                                                  data1: MIDIByte(lsb),
                                                  data2: MIDIByte(msb),
                                                  reserved: 0)
        MusicTrackNewMIDIChannelEvent(track, position.musicTimeStamp, &pitchingBendMessage)
    }

    /// Add pitching Bend reset to sequence
    ///
    /// - Parameters:
    ///   - position: Where in the sequence to insert pitchingbend info (expressed in beats)
    ///   - channel: MIDI channel to insert pitching bend reset on
    ///
    public func resetpitchingBend(position: Duration, channel: MIDIChannel = 0) {
        addpitchingBend(8192, position: position, channel: channel)
    }
