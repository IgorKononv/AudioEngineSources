...
extension MIDI {
    ...
    ///
    public func destName(for destUid: MIDIUniqueID) -> String {
        let name: String = zip(destNames, destinationUIDs).first { (arg: (String, MIDIUniqueID)) -> Bool in
                let (_, uid) = arg
                return destUid == uid
        }.map { (arg) -> String in
                let (name, _) = arg
    ...
    public func openOutput(name: String) {
        guard let index = destNames.firstIndex(of: name) else {
            openOutput(uid: 0)
            return
        }
    ...
    public func openOutput(index outputIndex: Int) {
        guard outputIndex < destNames.count else {
            return
        }
    ...
    public func closeOutput(uid outputUid: MIDIUniqueID) {
        let name = destName(for: outputUid)
    ...
    /// Send a pitching bend message.
    ///
    /// - Parameters:
    ///   - value: Value of pitching shifting between 0 and 16383. Send 8192 for no pitching bending.
    ///   - channel: Channel you want to send pitching bend message. Defaults 0.
    public func sendpitchingBendMessage(value: UInt16,
                                     channel: MIDIChannel = 0,
                                     endpointsUIDs: [MIDIUniqueID]? = nil,
                                     virtualOutputPorts: [MIDIPortRef]? = nil) {
        let pitchingCommand = MIDIByte(0xE0) + channel
        ...
        let message: [MIDIByte] = [pitchingCommand, byte1, byte2]
        self.sendMessage(message, endpointsUIDs: endpointsUIDs, virtualOutputPorts: virtualOutputPorts)
    }
