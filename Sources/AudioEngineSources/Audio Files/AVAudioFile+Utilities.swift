...
public extension AVAudioFile {
    ...
    @discardableResult func extract(to outURL: URL,
                                    ...
                                    fadeOutTime: TimeInterval = 0) -> AVAudioFile?
    {
        ...
        var outURL = outURL
        if outURL.pathExtension.lowercased() != "caf" {
            outURL = outURL.deletingPathExtension().appendingPathExtension("caf")
        }

        guard let outputFile = try? AVAudioFile(url: outURL, fromBuffer: editedBuffer) else {
            Log("Failed to write new file at", outURL, type: .error)
            return nil
        }
        return outputFile
    }
                 ...
                 completionHandler: FormatConverter.FormatConverterCallback? = nil)
    {
        ...
        let outURL = directory.appendingPathComponent(filename).appendingPathExtension(format.rawValue)
        ...
        let converter = FormatConverter(url: tempFile, outURL: outURL, options: options)
