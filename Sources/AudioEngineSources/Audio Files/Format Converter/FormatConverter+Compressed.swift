...
extension FormatConverter {
    ...
    func convertCompressed(presetName: String, completionHandler: FormatConverterCallback? = nil) {
        ...
        session.determineCompatibleFileTypes { list in
            ...
            guard let outputFileType: AVFileType = list.first else {
                let error = Self.createError(message: "Unable to determine a compatible file type from \(url.path)")
    ...
    func convertCompressed(presetName: String) async throws {
        ...
        guard let outputFileType: AVFileType = list.first else {
            throw Self.createError(message: "Unable to determine a compatible file type from \(url.path)")
        }
    ...
    func convertCompressed(completionHandler: FormatConverterCallback? = nil) {
        ...
        let tempName = outURL.deletingPathExtension().lastPathComponent + "_TEMP.wav"
        let tempFile = outURL.deletingLastPathComponent().appendingPathComponent(tempName)
        ...
        let tempConverter = FormatConverter(url: url,
                                            outURL: tempFile,
                                            options: tempOptions)
    ...
    func convertPCMToCompressed(completionHandler: FormatConverterCallback? = nil) {
        ...
        do {
            self.writer = try AVAssetWriter(outputURL: outURL, fileType: format)
        } catch let err as NSError {
            completionHandler?(err)
            return
        }
