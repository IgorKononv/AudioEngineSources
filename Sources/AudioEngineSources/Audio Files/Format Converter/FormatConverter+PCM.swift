...
extension FormatConverter {
    func convertToPCM(completionHandler: FormatConverterCallback? = nil) {
        ...
        if noErr != ExtAudioFileOpenURL(url as CFURL, &inputFile) {
        ...
        guard inputFormat != outputFormat ||
            ...
            Log("No conversion is needed, formats are the same. Copying to", outURL)
            // just copy it?
            do {
        ...
        if noErr != ExtAudioFileCreateWithURL(outURL as CFURL,
                                              format,
                                              &outputDescription,
                                              nil,
                                              ...
                                              &outputFile)
        {
            completionProxy(error: Self.createError(message: "Unable to create output file at \(outURL.path). " +
