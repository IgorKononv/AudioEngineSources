...
public extension AVAudioPCMBuffer {
    ...
    /// - Parameter strSmpl: Point to start copy from
    /// - Returns: an AVAudioPCMBuffer copied from a sample offset to the end of the buffer.
    func copyFrom(strSmpl: AVAudioFrameCount) -> AVAudioPCMBuffer? {
        guard strSmpl < frameLength,
              let buffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: frameLength - strSmpl)
        else {
            return nil
        }
        let framesCopied = buffer.copy(from: self, readOffset: strSmpl)
    ...
    /// - Returns: an AVAudioPCMBuffer copied from the start of the buffer to the specified endSmpl.
                 ...
                 to endTime: TimeInterval) -> AVAudioPCMBuffer?
    {
        ...
        let strSmpl = AVAudioFrameCount(startTime * sampleRate)
        var endSmpl = AVAudioFrameCount(endTime * sampleRate)

        if endSmpl == 0 {
            endSmpl = frameLength
        }

        let frameCapacity = endSmpl - strSmpl

        guard frameCapacity > 0 else {
            Log("strSmpl must be before endSmpl", type: .error)
            return nil
        }
        ...
        guard editedBuffer.copy(from: self, readOffset: strSmpl, frames: frameCapacity) > 0 else {
