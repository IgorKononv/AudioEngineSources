...
open class NodeRecorder: NSObject {
    ...
    {
        self.node = node
        self.filedirURL = filedirURL ?? URL(fileURLWithPath: NSTemporaryDirectory())
    ...
    public static func createAudioFile(filedirURL: URL = URL(fileURLWithPath: NSTemporaryDirectory())) -> AVAudioFile? {
        let filename = createDateFileName() + ".caf"
        let url = filedirURL.appendingPathComponent(filename)
    ...
    public func record() throws {
        ...
        if let path = internalAudioFile?.url.path, !FileManager.default.fileExists(atPath: path) {
            ...
            if let audioFile = NodeRecorder.createAudioFile(filedirURL: filedirURL) {
    ...
    public func createNewFile() {
        ...
        internalAudioFile = NodeRecorder.createAudioFile(filedirURL: filedirURL)
    }
}
